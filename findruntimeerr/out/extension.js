...
function activate(context) {
    try {
            ...
            if (config.pythonPath) {
                try {
                    ...
                    if (stats.isFile() || stats.isSymbolicLink()) {
                        ...
                        outputChannel.appendLine(`[getSelectedPythonPath] Path from settings is not a file or symlink: ${config.pythonPath}`);
                    }
                }
                catch (e) {
                    outputChannel.appendLine(`[getSelectedPythonPath] Path from settings is invalid: ${config.pythonPath} (Error: ${e.code})`);
                }
            }
            else {
                outputChannel.appendLine(`[getSelectedPythonPath] findRuntimeErr.pythonPath not set.`);
            }
            try {
                ...
                if (pythonExtension) {
                    ...
                    if (api?.environments?.resolveEnvironment &&
                        ...
                        const activeEnvPathDetails = api.environments.getActiveEnvironmentPath(resource);
                        const activeEnvPath = typeof activeEnvPathDetails === "string"
                        ...
                        if (activeEnvPath) {
                            ...
                            if (envPath) {
                                try {
                                    ...
                                    if (stats.isFile() || stats.isSymbolicLink()) {
                                        outputChannel.appendLine(`[getSelectedPythonPath] Using Python path from Python extension API (environments): ${envPath}`);
                                        return envPath;
                                    }
                                    else {
                                        outputChannel.appendLine(`[getSelectedPythonPath] Env path is not a file or symlink: ${envPath}`);
                                    }
                                }
                                catch (e) {
                                    outputChannel.appendLine(`[getSelectedPythonPath] Env path is invalid: ${envPath} (Error: ${e.code})`);
                                }
                            }
                            else {
                                outputChannel.appendLine(`[getSelectedPythonPath] Could not resolve environment path from active path: ${activeEnvPath}`);
                            }
                        }
                        else {
                            outputChannel.appendLine(`[getSelectedPythonPath] No active environment path returned by Python extension.`);
                        }
                    }
                        ...
                        if (potentialPath) {
                            try {
                                ...
                                if (stats.isFile() || stats.isSymbolicLink()) {
                                    outputChannel.appendLine(`[getSelectedPythonPath] Using Python path from Python extension API (settings): ${potentialPath}`);
                                    return potentialPath;
                                }
                                else {
                                    outputChannel.appendLine(`[getSelectedPythonPath] Settings path is not a file or symlink: ${potentialPath}`);
                                }
                            }
                            catch (e) {
                                outputChannel.appendLine(`[getSelectedPythonPath] Settings path is invalid: ${potentialPath} (Error: ${e.code})`);
                            }
                        }
                        else {
                            outputChannel.appendLine(`[getSelectedPythonPath] Could not get valid execution details from settings API.`);
                        }
                    }
                    else {
                        outputChannel.appendLine("[getSelectedPythonPath] Python extension API (environments/settings) not available.");
                    }
                }
                else {
                    outputChannel.appendLine("[getSelectedPythonPath] VSCode Python extension (ms-python.python) not found.");
                }
            }
            catch (err) {
            ...
            if (defaultPath) {
                try {
                    ...
                    if (stats.isFile() || stats.isSymbolicLink()) {
                        ...
                        outputChannel.appendLine(`[getSelectedPythonPath] defaultInterpreterPath is not a file or symlink: ${defaultPath}`);
                    }
                }
                catch (e) {
                    outputChannel.appendLine(`[getSelectedPythonPath] defaultInterpreterPath is invalid: ${defaultPath} (Error: ${e.code})`);
                }
            }
            else {
                outputChannel.appendLine(`[getSelectedPythonPath] python.defaultInterpreterPath not set.`);
            }
            outputChannel.appendLine("[getSelectedPythonPath] Falling back to 'python3' or 'python' command from PATH.");
        ...
        function checkPythonPackages(pythonExecutable) {
            ...
            let checkError;
            outputChannel.appendLine(`[checkPackages] Checking for: ${requiredPackages.join(", ")} using ${pythonExecutable}`);
            for (const pkg of requiredPackages) {
                try {
                    const command = `"${pythonExecutable}" -m pip show "${pkg}"`;
                    outputChannel.appendLine(`[checkPackages] Running: ${command}`);
                    (0, child_process_1.execSync)(command, { stdio: "pipe", encoding: "utf-8" });
                    outputChannel.appendLine(`[checkPackages] Package found: ${pkg}`);
                }
                catch (error) {
                    outputChannel.appendLine(`[checkPackages] Package not found or error checking: ${pkg}. Exit Code: ${error.status}`);
                    const stderr = error.stderr?.toString("utf-8") || "";
                    const stdout = error.stdout?.toString("utf-8") || "";
                    if (stderr)
                        outputChannel.appendLine(`[checkPackages] Stderr: ${stderr.substring(0, 200)}...`);
                    if (stdout)
                        outputChannel.appendLine(`[checkPackages] Stdout: ${stdout.substring(0, 200)}...`);
                    ...
                    if (error.status === 127 ||
                        ...
                        outputChannel.appendLine(`[checkPackages] Critical Python execution failure: ${checkError}`);
                        break;
                    }
                }
            }
        ...
        async function runAnalysisProcess(code, mode, documentUri) {
            ...
            try {
                ...
                outputChannel.appendLine(`[runAnalysisProcess] Failed to get Python path: ${e.message}`);
            ...
            if (lastUsedPythonExecutable !== pythonExecutable) {
                outputChannel.appendLine(`[runAnalysisProcess] Using Python: ${pythonExecutable}`);
            ...
            return new Promise((resolve) => {
                try {
                    ...
                    if (proceedAnalysis) {
                        outputChannel.appendLine(`[runAnalysisProcess] Checking required packages...`);
                        ...
                        if (checkResult.error || checkResult.missing.length > 0) {
                            const errorMsg = checkResult.error ||
                                `FindRuntimeErr requires: ${checkResult.missing.join(", ")}. Please run 'pip install parso astroid networkx' in '${pythonExecutable}'. Analysis skipped.`;
                        ...
                        checkedPackages = true;
                        outputChannel.appendLine(`[runAnalysisProcess] Required packages check passed.`);
                    }
                    else {
                        outputChannel.appendLine(`[runAnalysisProcess] Skipping package check (already passed).`);
                    }
                    ...
                    outputChannel.appendLine(`[runAnalysisProcess] Spawning: "${pythonExecutable}" "${scriptPath}" ${mode}`);
                    const pythonProcess = (0, child_process_1.spawn)(pythonExecutable, [scriptPath, mode], spawnOptions);
                    let stdoutData = "";
                    let stderrData = "";
                    pythonProcess.stdin.write(code, "utf-8");
                    pythonProcess.stdin.end();
                    outputChannel.appendLine(`[runAnalysisProcess] Sent code (${Buffer.byteLength(code, "utf-8")} bytes) to Python stdin.`);
                    ...
                    pythonProcess.on("close", (closeCode) => {
                        try {
                            ...
                            if (closeCode !== 0 ||
                                ...
                                let errorLine = 1, errorColumn = 0, errorToLine = 1, errorEndColumn = 1;
                                ...
                                try {
                                    ...
                                    outputChannel.appendLine(`[runAnalysisProcess] Failed to parse stdout as error JSON: ${e}`);
                                }
                                const trimmedStderr = stderrData.trim();
                                if (trimmedStderr &&
                                    !errorDetail.includes(trimmedStderr.substring(0, 100))) {
                                    errorDetail += `\nStderr: ${trimmedStderr.substring(0, 500)}...`;
                                }
                                resolve({
                                    errors: [
                                        {
                            ...
                            try {
                                ...
                                // JSON 파싱 자체 실패 시
                                outputChannel.appendLine(`[runAnalysisProcess] Error parsing analysis results: ${parseError.message}. Raw: ${trimmedStdout.substring(0, 500)}...`); // 로그 길이 증가
                                resolve({
                                    errors: [
                                        {
                            ...
                            if (result &&
                                ...
                                outputChannel.appendLine(`[runAnalysisProcess] Invalid analysis result format after parse: ${JSON.stringify(result).substring(0, 200)}...`);
                                resolve({
                                    errors: [
                                        {
                                            message: "Invalid analysis result format received after successful parse.",
                                            line: 1,
                                            column: 0,
                                            to_line: 1,
                                            end_column: 1,
                        ...
                        catch (processCloseError) {
                            // on('close') 콜백 자체의 오류 처리
                            outputChannel.appendLine(`[runAnalysisProcess] Error in on('close') handler: ${processCloseError.message}`);
                            resolve({
                                errors: [
                                    {
        ...
        async function runDynamicAnalysisProcess(code, documentUri) {
            ...
            try {
                ...
                outputChannel.appendLine(`[runDynamicAnalysisProcess] Failed to get Python path: ${e.message}`);
                return Promise.resolve({
                    errors: [
                        {
            ...
            if (!fs.existsSync(scriptPath)) {
                outputChannel.appendLine(`[runDynamicAnalysisProcess] Error: dynamic_analyze.py not found: ${scriptPath}`);
                return Promise.resolve({
                    errors: [
                        {
            ...
            return new Promise((resolve) => {
                ...
                const process = (0, child_process_1.spawn)(pythonExecutable, [scriptPath], spawnOptions);
                let stdoutData = "";
                let stderrData = "";
                ...
                process.on("close", (closeCode) => {
                    outputChannel.appendLine(`[runDynamicAnalysisProcess] Process exited with code: ${closeCode}`);
                    if (closeCode !== 0 || stderrData.trim()) {
                        const errorMsg = `Dynamic analysis script failed (Code: ${closeCode}). ${stderrData
                            .trim()
                            .substring(0, 500)}...`;
                        outputChannel.appendLine(`[runDynamicAnalysisProcess] Error: ${errorMsg}`);
                        resolve({
                            errors: [
                                {
                                    message: errorMsg,
                    ...
                    try {
                        ...
                        if (result && Array.isArray(result.errors)) {
                            ...
                            outputChannel.appendLine(`[runDynamicAnalysisProcess] Invalid result format: ${trimmedStdout.substring(0, 200)}...`);
                            resolve({
                                errors: [
                                    {
                        ...
                        outputChannel.appendLine(`[runDynamicAnalysisProcess] Error parsing result: ${e}. Raw: ${trimmedStdout.substring(0, 200)}...`);
                        resolve({
                            errors: [
                                {
                ...
                process.on("error", (err) => {
                    outputChannel.appendLine(`[runDynamicAnalysisProcess] Failed to start process: ${err.message}`);
                    resolve({
                        errors: [
                            {
        ...
        async function analyzeCodeRealtime(document) {
            ...
            try {
                ...
                clearPreviousAnalysis(documentUri);
                const analysisResult = await runAnalysisProcess(code, "realtime", documentUri);
                ...
                handleAnalysisResult(document.uri, getConfiguration(), {
                    errors: [
                        {
        ...
        function handleAnalysisResult(documentUri, config, result, mode) {
            ...
            const filteredErrors = result.errors.filter((err) => {
                const isValidErrorObject = err &&
                    typeof err.message === "string" &&
                    typeof err.line === "number" &&
                    typeof err.column === "number" &&
                    typeof err.errorType === "string";
                if (!isValidErrorObject) {
                    outputChannel.appendLine(`[handleAnalysisResult] Invalid error object found: ${JSON.stringify(err)}`);
                    return false;
                }
                const isIgnored = config.ignoredErrorTypes.includes(err.errorType.toLowerCase());
                if (isIgnored) {
                    outputChannel.appendLine(`[handleAnalysisResult] Ignored error by type: ${err.errorType}`);
                }
                return !isIgnored;
            });
            ...
            errors.forEach((err) => {
                try {
                    ...
                    if (severity <= config.severityLevel) {
                        ...
                        outputChannel.appendLine(`[displayDiagnostics] Ignored by severity setting: ${err.errorType} (Severity: ${severity}, Config: ${config.severityLevel})`);
                    }
                }
                catch (e) {
                    outputChannel.appendLine(`[displayDiagnostics] Error processing item: ${JSON.stringify(err)} - ${e.message}`);
                }
            });
            outputChannel.appendLine(`[displayDiagnostics] Displaying ${displayedCount} diagnostics out of ${errors.length} filtered errors.`);
            ...
            provideHover(document, position, token) {
                try {
                    ...
                    diagnosticsAtPos.slice(0, 5).forEach((diagnostic, index) => {
                        ...
                        hoverContent.appendMarkdown(`**[${diagnostic.code || "FindRuntimeErr"}]** ${diagnostic.message}`);
                    });
        ...
        const triggerRealtimeAnalysis = (document) => {
            ...
            debounceTimeout = setTimeout(() => {
                analyzeCodeRealtime(document).catch((e) => {
                    ...
                    outputChannel.appendLine(`ERROR in analyzeCodeRealtime (triggered): ${e.message}`);
                });
            }, debounceDelay);
        };
        ...
        context.subscriptions.push(vscode.workspace.onDidChangeConfiguration((e) => {
            ...
            if (e.affectsConfiguration("findRuntimeErr")) {
                outputChannel.appendLine("[Config Change] findRuntimeErr config changed.");
            ...
            if (e.affectsConfiguration("python.defaultInterpreterPath")) {
                outputChannel.appendLine("[Config Change] python.defaultInterpreterPath changed.");
                resetPackageCheck = true;
                needsReanalysis = true;
            }
        ...
        context.subscriptions.push(vscode.commands.registerCommand("findRuntimeErr.analyzeCurrentFile", async () => {
            try {
                ...
                if (editor?.document.languageId === "python") {
                    outputChannel.appendLine("[Command] analyzeCurrentFile triggered.");
                    ...
                    await vscode.window.withProgress({
                        ...
                        vscode.window.showInformationMessage(`FindRuntimeErr: 정적 분석 완료. ${result.errors.length}개 이슈 발견.`);
                    });
                }
                else {
                    vscode.window.showWarningMessage("정적 분석할 Python 파일을 여세요.");
                }
            }
            catch (e) {
                outputChannel.appendLine(`[Command Error] analyzeCurrentFile: ${e.message}\n${e.stack}`);
        ...
        context.subscriptions.push(vscode.commands.registerCommand("findRuntimeErr.runDynamicAnalysis", async () => {
            try {
                ...
                if (editor && editor.document.languageId === "python") {
                    outputChannel.appendLine("[Command] runDynamicAnalysis triggered.");
                    const config = getConfiguration();
                    if (!config.enableDynamicAnalysis) {
                        vscode.window.showInformationMessage("FindRuntimeErr: 동적 분석 비활성화됨 ('findRuntimeErr.enableDynamicAnalysis').");
                        return;
                    }
                    clearPreviousAnalysis(editor.document.uri);
                    await vscode.window.withProgress({
                        ...
                        vscode.window.showInformationMessage(`FindRuntimeErr: 동적 분석 완료. ${result.errors.length}개 오류/실패 발견.`);
                    });
                }
                else {
                    vscode.window.showWarningMessage("동적 분석할 Python 파일을 여세요.");
                }
            }
            catch (e) {
                outputChannel.appendLine(`[Command Error] runDynamicAnalysis: ${e.message}\n${e.stack}`);
        ...
        async function runInitialAnalysis() {
            ...
            if (editor &&
                editor.document.languageId === "python" &&
                editor.document.uri.scheme === "file") {
                outputChannel.appendLine("[Activate] Running initial analysis for active Python file.");
                try {
                    const initialConfig = getConfiguration();
                    const initialPython = await getSelectedPythonPath(editor.document.uri);
                    const pkgsCheck = checkPythonPackages(initialPython);
                    if (pkgsCheck.error || pkgsCheck.missing.length > 0) {
                        const errorMsg = pkgsCheck.error ||
                            `Missing packages: ${pkgsCheck.missing.join(", ")}. Run 'pip install parso astroid networkx'`;
                ...
                outputChannel.appendLine("[Activate] No active local Python editor for initial analysis.");
            }
        }
        runInitialAnalysis().catch((e) => {
            console.error("Error executing runInitialAnalysis:", e);
            outputChannel?.appendLine(`ERROR executing runInitialAnalysis: ${e.message}`);
        });
    }
    catch (e) {
        console.error("FATAL Error during extension activation:", e);
        vscode.window.showErrorMessage(`FindRuntimeErr failed to activate: ${e.message}. Check output channel.`);
        if (outputChannel) {
            outputChannel.appendLine(`FATAL Error during activation: ${e.message}\n${e.stack}`);
        }
    }
}
